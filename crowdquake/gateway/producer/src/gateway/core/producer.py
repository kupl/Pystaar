...
class KafkaProducer(Producer):
  ...
  def __init__(self, configs: dict):
    ...
    prod_params = {
      ...
      'batch.num.messages': self._batch_num_msg
    }
  ...
  def produce(self, topic: str, key: bytes, value: bytes, on_delivery=None) -> None:
    ...
    self._producer.poll(0)  # handle delivered results.

  def produce_with_sensor_timestamp(self, topic: str, key: bytes, value: bytes, timestamp: int,
                                    on_delivery=None) -> None:
    headers = {
      'timestamp': timestamp.to_bytes(8, 'little', signed=False)
    }
    self._producer.produce(topic=topic, key=key,
                           value=value,
                           on_delivery=on_delivery,
