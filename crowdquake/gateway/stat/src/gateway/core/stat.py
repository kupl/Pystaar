...
class Stat:
  def __init__(self, num_window_size: int, sampling: int = 1, reporting_interval_sec: int = 5,
  ...
  def print_window(self):
    ...
    stats = {
      'produced_msg_cnt': self._window_count,
      'elapsed_sec': f"{elapsed:.4f}",
      'max_latency_sec': f"{self._window_max_latency:.4f}",
      'mean_latency_sec': f"{self._window_total_latency / self._window_count:.4f}",
      'bytes_produced': f"{self._window_bytes / 1024 / 1024 / elapsed:.4f} MB/sec",
      'percentiles': {
        '50p': f'{q[0]:.4f}',
        '75p': f'{q[1]:.4f}',
        '90p': f'{q[2]:.4f}',
