...
from llm_test_generation.tools.base import ToolBase, ToolBaseBuilder

from llm_test_generation.util.filesys import make_directory, write_file
from llm_test_generation.util.logger import Logger, LoggerName
from llm_test_generation.util.codeinfo import CodeInfo
...
class TestGenerator(ToolBase):
  ...
  def _is_valid(self, out: Function, fct: str, **kwargs) -> bool:
    ...
    if search('(?:= |\n|\t|\.)\_[a-zA-Z0-9_]+', out.code): 
      return False
    # 통과 가능한 오류 타입이 아니면 유효하지 않음.
    elif out.result.type not in self.pass_type: 
      return False
