...
from llm_test_generation.tools.test_generator.pos_test_generator import PositiveTestGeneratorBuilder
from llm_test_generation.util.filesys import make_directory, read_json, write_file, write_json
...
class Default(Enum):
  Model = "response"
  Framework = "pytest"
  Model_Config_Path = Path("configs/openai/response/pos_test_generator.json")
  Framework_Config_Path = Path()
  Out_DirPath = Path("out")


# 코드 경로 리스트 src, 사용자 정의 정보 딕셔너리 res, 함수 fct에 대하여
...
def run(src: list[Path], lines: list[ErrorLine], res={}, iter=1, cand=3, n=3,
  ...
  fct = lines[0].method
  generator = (PositiveTestGeneratorBuilder()
               .add_pass_type("None")
               .set_paths(src)
               .set_res(res)
               .set_iteration(iter)
  ...
  logger.info("Running positive testcase generation")
  generated = generator.run(fct=fct)

  # 생성한 테스트가 없으면 빈 리스트 반환.
  if not generated: return []

  # 테스트 이름 재설정.
  for i in range(len(generated)):
    generated[i].name = f"test_pos_{i+1}"
  return generated[:min(n, len(generated))]
...
def main():
  ...
  # parser.add_argument("-f", "--fct", metavar="FUNCTION_NAME", type=str, required=True,
  #                     help="function name")
  ...
  parser.add_argument("-mc", "--model-config", metavar="MODEL_CONFIG_PATH", type=Path,
  ...
  pos_code_path = tests_dirpath/"pos.py"
  pos_json_path = tests_dirpath/"pos.json"
  make_directory(tests_dirpath)
  write_file(pos_code_path, "\n\n".join(test.to_py() for test in testcases))
  write_json(pos_json_path, {"codes": [test.to_dict() for test in testcases]})
